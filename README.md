...
The v8.1.0 APK is a minified release build with unused resources removed. It is signed with a local development certificate because this fork does not have a release keystore configured; it installs as a fresh app, but cannot update an installation signed by Google Play or another key.

## App size

The v8 rewrite bundles about 20 selectable Greek fonts; v7 bundled one. The packaged database also grew from 7,979,008 bytes to 15,794,176 bytes. These app assets account for much of the larger v8 package. The initial 41 MB APK from this checkout was an unshrunk debug build. The optimized v8.1.0 release APK is about 21 MB with R8 code shrinking and resource shrinking enabled.
